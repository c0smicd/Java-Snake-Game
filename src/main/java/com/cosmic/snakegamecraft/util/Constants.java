package com.cosmic.snakegamecraft.util;

public class Constants {

    public static final int TILE_SIZE = 32; // Size of each tile in pixels
    public static final int GRID_SIZE = 20; // Number of tiles in the grid width
    public static final int STANDARD_SPEED = 3; // Standard Multiplier for game speed


    public static final double BAD_APPLE_CHANCE = 25; // Chance of a bad apple appearing
    public static final double RAINBOW_APPLE_CHANCE = 10; // Chance of a rainbow apple appearing


    public static final int INVULNERABILITY_DURATION = 100; // Duration of invulnerability in ticks
    public static final int INITIAL_SNAKE_LENGTH = 3; // Initial length of the snake

    public static final int SHOWN_HIGHSCORES = 5; // Number of high scores to show in the leaderboard

}
